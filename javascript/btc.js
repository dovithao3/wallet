...
btc.extend = function (pass) {
    ...
    for (var i = 0; i < 40; i++) {
        pass = pass.concat(SHA256(pass), RIPEMD160(pass)).reverse();
    }
...
btc.new_script = function (data) {
    data = data || '';
    if (typeof data != 'string') throw 'new_script() needs a string'; //s.buffer = data;

    var s = {buffer: btc.hex2bin(data)};

    s.write_opcode = function (op) {
        this.buffer.push(op);
        this.chunks.push(op);
        return true;
    };
    ...
    s.parse = function () {
        ...
        for (var i = 0; i < this.buffer.length;) {
            ...
            if (opcode > 78) {
                ...
                if (opcode >= 78) {
                    len = (len << 8) | this.buffer[i++];   // OP_PUSHDATA4
                    len = (len << 8) | this.buffer[i++];
                }

                this.chunks.push(this.buffer.slice(i, i + len));
                i += len; // read_chunk(len);
...
btc.new_tx = function () {
    ...
    {
        var b = this.serialize(idx).concat(btc.n2bytes(1, 4)); // signature type?
    ...
    tx.sign_input = function (idx, sk) {
        ...
        var half_n = n.shiftRight(1);
        if (s.compareTo(half_n) > 0) s = n.subtract(s); // Force lower s values per BIP62

        return serialize_sig(r, s).concat([1]);
    };//_______________________________________________________________________
    ...
    tx.txid = function () {
        return btc.bin2hex(SHA256(SHA256(this.serialize())).reverse());
    };
